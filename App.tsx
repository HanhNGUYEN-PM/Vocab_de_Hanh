...
const REMOTE_SYNC_ENDPOINT = 'https://jsonbase.com/vocab-de-hanh/shared-collection';
...
const App: React.FC = () => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          if (localRevisionRef.current > 0 && vocabulary.length > 0) {
            void pushRemoteVocabulary(vocabulary, localRevisionRef.current);
          }
          return;
        }
        throw new Error(`HTTP ${response.status}`);
      ...
      let remoteItems: VocabularyItem[] = [];
      let remoteRevision = 0;

      if (candidate) {
        if (Array.isArray(candidate)) {
          remoteItems = candidate as VocabularyItem[];
        } else if (Array.isArray((candidate as PersistedPayload).vocabulary)) {
          remoteItems = (candidate as PersistedPayload).vocabulary;
          if (typeof (candidate as PersistedPayload).updatedAt === 'number') {
            remoteRevision = (candidate as PersistedPayload).updatedAt;
          }
        }
      ...
      const effectiveRevision = remoteRevision || Date.now();
  ...
  }, [applyRemoteVocabulary, isSyncReady, pushRemoteVocabulary, vocabulary]);
