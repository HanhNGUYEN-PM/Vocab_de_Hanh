...
const App: React.FC = () => {
  ...
  const [activeSyncTab, setActiveSyncTab] = useState<'create' | 'import'>('create');
  ...
  const handleSelectSyncTab = (tab: 'create' | 'import') => {
    setActiveSyncTab(tab);
    setIsSyncCollapsed(false);
    if (tab === 'import') {
      setImportInput('');
      setImportError(null);
        ...
        <div className="flex flex-col lg:flex-row lg:items-start gap-6">
          ...
          >
            <div className="flex flex-col gap-4">
              ...
              <div className="inline-flex rounded-md border border-indigo-200 bg-white/60 p-1">
                <button
                  type="button"
                  onClick={() => handleSelectSyncTab('create')}
                  className={`px-3 py-2 text-sm font-semibold rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${
                    activeSyncTab === 'create'
                      ? 'bg-white text-indigo-700 shadow-sm'
                      : 'text-indigo-600 hover:bg-indigo-100'
                  }`}
                >
                  Choisir votre code de synchronisation
                </button>
                <button
                  type="button"
                  onClick={() => handleSelectSyncTab('import')}
                  className={`px-3 py-2 text-sm font-semibold rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${
                ...
                >
                  Saisir un code de synchronisation
                </button>
              </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide">
                          Choisissez un code de 6 chiffres
                        </label>
                        <div className="flex flex-col sm:flex-row gap-2">
                          <input
                            ...
                            placeholder="Par exemple : 246810"
                          />
                          <button
                            type="button"
                            onClick={() => {
                        ...
                        <p className="text-xs text-slate-500">
                          Choisissez des chiffres faciles à retenir. Lorsque vous copiez le code, votre vocabulaire est caché dans ces six chiffres.
                        </p>
                        {customCodeError && (
                      ...
                      >
                        Copier ce code pour le partager
                      </button>
                        ...
                        <div className="space-y-2">
                          <div>
                            ...
                            <p className="text-xs text-slate-500 mt-2">
                              Utilisez le bouton de copie pour conserver les données cachées avec vos chiffres. Collez le code sur l'autre appareil pour restaurer vos mots.
                            </p>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            >
                              {isPackageVisible ? 'Masquer le texte de secours' : 'Afficher le texte de secours (package complet)'}
                            </button>
                            {isPackageVisible && (
                              <div>
                                <textarea
                                ...
                                />
                                <p className="text-xs text-slate-500 mt-2">
                                  Copiez et envoyez cette chaîne uniquement si le collage des six chiffres ne fonctionne pas. L'autre appareil doit recevoir tout le texte.
                                </p>
                              </div>
                            )}
                          </div>
                  ...
                  {activeSyncTab === 'import' && (
                    <form onSubmit={handleImportSubmit} className="space-y-4">
                      <div className="space-y-2">
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide">
                          Saisissez un code de 6 chiffres pour vous synchroniser
                        </label>
                        <input
                          value={importInput}
                          onChange={(event) => {
                          ...
                          inputMode="numeric"
                          pattern="[0-9]*"
                          placeholder="Collez ou saisissez votre code (exemple : 246810)"
                        ...
                        />
                        <p className="text-xs text-slate-500">
                          Collez les chiffres copiés. La copie conserve les données cachées, mais vous pouvez aussi taper les six chiffres si l'application a retiré les caractères invisibles.
                        </p>
                        <button
                          type="button"
                          onClick={() => {
                        ...
                        >
                          {isManualImportMode ? 'Utiliser uniquement les 6 chiffres' : 'Besoin d'aide ? Collez le package complet'}
                        </button>
                      </div>
                      ...
                      {isManualImportMode && (
                        <div className="space-y-2">
                          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide">
                            Collez le texte complet de synchronisation
                          </label>
                          <textarea
                            value={importInput}
                            onChange={(event) => {
                            ...
                            placeholder="Collez la chaîne complète (exemple : 123456:eyJ2b2NhYiI6Wy4uLl0=)"
                          ...
                          />
                          <p className="text-xs text-slate-500">
                            Utilisez cette solution si une application de messagerie supprime les données cachées des chiffres.
                          </p>
                        </div>
                      )}
